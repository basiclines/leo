...
var elementFetchRender = function (_LEOElement) {
	_inherits(elementFetchRender, _LEOElement);

	function elementFetchRender() {
		_classCallCheck(this, elementFetchRender);

		return _possibleConstructorReturn(this, (elementFetchRender.__proto__ || Object.getPrototypeOf(elementFetchRender)).apply(this, arguments));
	}

	_createClass(elementFetchRender, [{
		key: 'fetchData',
		...
		value: function render() {
			...
			if (!this.data.isEmpty) {
				this.innerHTML = this.data.users.reduce(function (buffer, item) {
					return buffer += _this3.renderItem(item);
				}, '');
			}
		}
	}]);

	return elementFetchRender;
}(_leo.LEOElement);
